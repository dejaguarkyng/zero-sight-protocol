 ...
 */
export function createTimestampedPayload(data: string, clientSecret: string): string {
  const encrypted = encryptWithSecret(clientSecret, data)
  const payload: TimestampedPayload = {
 ...
 */
export function validateTimestampedPayload(
  encryptedPayload: string,
  clientSecret: string,
  timeWindowMs = 300000,
): string {
  const payloadBuffer = Buffer.from(encryptedPayload, "base64")
  const payload = JSON.parse(payloadBuffer.toString("utf8")) as TimestampedPayload
